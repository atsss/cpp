...
void removeDuplicates1( Node * head ) {
  ...
  while( curr != nullptr ) {
    while (curr && node_map.find(curr->data) != node_map.end()) { curr = curr->next; }
...
int main() {
  std::cout << "Method 1 : \n";
  Node * head = nullptr;
  for ( int i = 0; i < 10; ++i ) { insert(head, random_range(1,7)); }
  ...
  std::cout << "Method 2 : \n";
  Node * head1 = nullptr;
  for ( int i = 0; i < 10; ++i ) { insert(head1, random_range(1,7)); }
