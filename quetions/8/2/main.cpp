...
bool getPath(int** matrix, int currRow, int currColumn, vector<Point*>& path, unordered_set<Point*>& duplicateVisits) {
  ...
  if (currRow < 0 || currColumn < 0 || matrix[currRow][currColumn] == -1) { return false; }
...
vector<Point*> getPath(int** matrix, int rows, int columns) {
  ...
  if (rows != 0 || matrix != nullptr) {
    ...
    if (getPath(matrix, rows - 1, columns - 1, path, duplicateVisits)) { return path; }
  }
...
int main() {
  ...
  if (path.size() == 0) { cerr << "Path does not exist!" << endl; }
  else {
